# Скриптонайзер Lite (офисная версия)
...
class ScriptonizerLite:
    ...
    def setup_ui(self):
        ...
        tk.Button(top_frame, text="⚙️", width=4, height=2, command=self.show_about).pack(side=tk.LEFT, padx=2)
        tk.Button(top_frame, text="👁️", width=4, height=2, command=self.toggle_hide).pack(side=tk.LEFT, padx=2)
        tk.Button(top_frame, text="➖", width=4, height=2, command=self.minimize_to_button).pack(side=tk.LEFT, padx=2)
        
        self.top_btn = tk.Button(top_frame, text="📌", width=4, height=2, command=self.toggle_always_on_top, relief=tk.RAISED)
        self.top_btn.pack(side=tk.LEFT, padx=2)
        
        tk.Button(top_frame, text="📝", width=4, height=2, command=self.open_editor).pack(side=tk.LEFT, padx=2)
        
        tk.Button(top_frame, text="🔍", width=3, height=2, command=self.focus_search).pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(top_frame, text="➕", width=4, height=2, bg="#4CAF50", fg="white", command=self.add_script).pack(side=tk.RIGHT, padx=2)
    ...
    def toggle_always_on_top(self):
        ...
        if self.always_on_top:
            ...
            self.top_btn.config(relief=tk.SUNKEN, bg="#ffcc00")
        else:
            self.root.attributes('-topmost', False)
            self.top_btn.config(relief=tk.RAISED, bg="SystemButtonFace")
    ...
    def open_editor(self):
        ...
        def delete_selected():
            ...
            if sel and messagebox.askyesno("Удалить", f"Удалить скрипт '{self.scripts[sel[0]]['name']}'?"):
        ...
        tk.Button(mid_btn_frame, text="🗑️ Удалить", command=delete_selected, bg="#f44336", fg="white", width=12).pack(side=tk.LEFT, padx=5)
