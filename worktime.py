...
class WorkTimeTracker:
    ...
    def print_timesheet(self):
        """Печать текущего табеля"""
        content = self.timesheet_text.get('1.0', 'end')
        if not content.strip():
            messagebox.showwarning("Внимание", "Сначала сформируйте табель!")
            return
        
        temp_file = "temp_timesheet.txt"
        with open(temp_file, 'w', encoding='utf-8') as f:
            f.write(content)
        
        try:
            os.startfile(temp_file, 'print')
            messagebox.showinfo("Печать", "Отправлено на печать")
        except:
            try:
                subprocess.run(['notepad', '/p', temp_file], check=True)
            except:
                messagebox.showinfo("Печать", "Сохраните табель в Excel и распечатайте вручную")
