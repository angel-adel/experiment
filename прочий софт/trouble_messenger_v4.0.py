...
# ========== НАСТРОЙКА ЛОГИРОВАНИЯ (только локально) ==========
LOG_DIR = "Logs"
...
log_info("Программа запущена (Trouble Messenger 4.0)")
...
# ========== ПОСТРОЕНИЕ HTML-ОТЧЁТОВ (без почты в футере) ==========
...
<div class="container">
    ...
    <hr><p style="text-align:center;">📧 Отправьте этот отчёт разработчикам (через программу или вручную)</p>
</div>
</body>
</html>"""
    else:
...
def save_offline_report(description, mode):
    ...
    try:
        ...
        log_info(f"Отчёт сохранен: {filepath} (режим {mode})")
...
class TroubleMessenger:
    def __init__(self, root):
        self.root = root
        self.root.title("Trouble Messenger 4.0 — Ушастик")
        ...
        tk.Label(root, text="Пароль почтового приложения приложения (Outlook, gmail и т. д.):", font=("Arial", 10)).pack(pady=(5,0))
        ...
        log_info("Интерфейс программы загружен (Trouble Messenger 4.0)")
